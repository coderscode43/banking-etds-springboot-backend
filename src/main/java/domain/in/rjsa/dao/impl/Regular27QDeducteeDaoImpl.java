...
//			criteria.add(Restrictions.eqOrIsNull("roCode", entity.get("roCode").toString().toString()));
//		}
//
//	}
//}
...
public class Regular27QDeducteeDaoImpl extends AbstractDaoFY<Long, Regular27QDeductee>
    ...
    public List<Regular27QDeductee> search(HashMap entity, int pageNo, int noOfResult) {
        ...
        CriteriaQuery<Regular27QDeductee> cq = cb.createQuery(Regular27QDeductee.class);
        Root<Regular27QDeductee> root = cq.from(Regular27QDeductee.class);

        cq.select(root).where(buildPredicates(cb, root, entity));
        cq.orderBy(cb.desc(root.get("id")));

        TypedQuery<Regular27QDeductee> query = getEntityManager().createQuery(cq);
    ...
    private Predicate[] buildPredicates(CriteriaBuilder cb, Root<Regular27QDeductee> root, Map entity) {
