...
public class RegularReturnExcel {
...
//				row3.getCell(i).setCellStyle(style2);
			...
			row1.getCell(1).setCellValue("Regular Return");
...
//			row4.getCell(5).setCellValue("");
	...
	private void studentDetailExcel(SXSSFWorkbook wbs2) {
		try {
			int part = 1;
			initializeSheet("RegularReturn-1");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
//		stylematchblank();
		try {
			Sheet sheet = wbs.createSheet("RegularReturn-1");
			...
			row0.getCell(1).setCellValue("Date");
			sheet.setColumnWidth(2, 5000);
			...
			row0.getCell(6).setCellValue("Added By");
			sheet.setColumnWidth(7, 4000);
			row0.getCell(7).setCellValue("Latest Remark");
			sheet.setColumnWidth(8, 4000);
			row0.getCell(8).setCellValue("Status");
			sheet.setColumnWidth(9, 4000);
			row0.getCell(9).setCellValue("Return Filing Date");
	...
	public static void main(String[] args) {
		RODetailsExcel cde = new RODetailsExcel();
		cde.initialise("C:\\Users\\RJSA-15022021-03\\Desktop\\RODetails.xlsx");
		cde.close();
	}
