!function(v){var u,t,s="0.4.2",r="hasOwnProperty",q=/[\.\/]/,p="*",o=function(){},n=function(d,c){return d-c
},m={n:{}},l=function(E,D){E=String(E);
var C,B=t,A=Array.prototype.slice.call(arguments,2),z=l.listeners(E),y=0,x=[],w={},k=[],i=u;
u=E,t=0;
for(var c=0,b=z.length;
b>c;
c++){"zIndex" in z[c]&&(x.push(z[c].zIndex),z[c].zIndex<0&&(w[z[c].zIndex]=z[c]))
}for(x.sort(n);
x[y]<0;
){if(C=w[x[y++]],k.push(C.apply(D,A)),t){return t=B,k
}}for(c=0;
b>c;
c++){if(C=z[c],"zIndex" in C){if(C.zIndex==x[y]){if(k.push(C.apply(D,A)),t){break
}do{if(y++,C=w[x[y]],C&&k.push(C.apply(D,A)),t){break
}}while(C)
}else{w[C.zIndex]=C
}}else{if(k.push(C.apply(D,A)),t){break
}}}return t=B,u=i,k.length?k:null
};
l._events=m,l.listeners=function(F){var E,D,C,B,A,z,y,x,w=F.split(q),j=m,g=[j],f=[];
for(B=0,A=w.length;
A>B;
B++){for(x=[],z=0,y=g.length;
y>z;
z++){for(j=g[z].n,D=[j[w[B]],j[p]],C=2;
C--;
){E=D[C],E&&(x.push(E),f=f.concat(E.f||[]))
}}g=x
}return f
},l.on=function(h,f){if(h=String(h),"function"!=typeof f){return function(){}
}for(var w=h.split(q),k=m,j=0,i=w.length;
i>j;
j++){k=k.n,k=k.hasOwnProperty(w[j])&&k[w[j]]||(k[w[j]]={n:{}})
...
},l.nt=function(b){return b?new RegExp("(?:\\.|\\/|^)"+b+"(?:\\.|\\/|$)").test(u):u
},l.nts=function(){return u.split(q)
},l.off=l.unbind=function(C,B){if(!C){return l._events=m={n:{}},void 0
}var A,z,y,x,w,k,j,g=C.split(q),f=[m];
for(x=0,w=g.length;
w>x;
x++){for(k=0;
k<f.length;
k+=y.length-2){if(y=[k,1],A=f[k].n,g[x]!=p){A[g[x]]&&y.push(A[g[x]])
}else{for(z in A){A[r](z)&&y.push(A[z])
}}f.splice.apply(f,y)
}}for(x=0,w=f.length;
w>x;
x++){for(A=f[x];
A.n;
){if(B){if(A.f){for(k=0,j=A.f.length;
j>k;
k++){if(A.f[k]==B){A.f.splice(k,1);
break
}}!A.f.length&&delete A.f
}for(z in A.n){if(A.n[r](z)&&A.n[z].f){var e=A.n[z].f;
for(k=0,j=e.length;
j>k;
k++){if(e[k]==B){e.splice(k,1);
break
}}!e.length&&delete A.n[z].f
}}}else{delete A.f;
for(z in A.n){A.n[r](z)&&A.n[z].f&&delete A.n[z].f
}}A=A.n
}}},l.once=function(e,d){var f=function(){return l.unbind(e,f),d.apply(this,arguments)
};
return l.on(e,f)
},l.version=s,l.toString=function(){return"You are running Eve "+s
...
}}}function a1(f,e,h){function g(){var d=Array.prototype.slice.call(arguments,0),c=d.join("â�?€"),b=g.cache=g.cache||{},a=g.count=g.count||[];
...
}function aT(B,A,z,y,x,w,v,u,t){if(!(0>t||aU(B,A,z,y,x,w,v,u)<t)){var s,r=1,q=r/2,p=r-q,j=0.01;
for(s=aU(B,A,z,y,x,w,v,u,p);
bp(s-t)>j;
){q/=2,p+=(t>s?1:-1)*q,s=aU(B,A,z,y,x,w,v,u,p)
}return p
}}function aR(D,C,B,A,z,y,x,w){if(!(bs(D,B)<br(z,x)||br(D,B)>bs(z,x)||bs(C,A)<br(y,w)||br(C,A)>bs(y,w))){var v=(D*A-C*B)*(z-x)-(D-B)*(z*w-y*x),u=(D*A-C*B)*(y-w)-(C-A)*(z*w-y*x),t=(D-B)*(y-w)-(C-A)*(z-x);
if(t){var s=v/t,r=u/t,q=+s.toFixed(2),p=+r.toFixed(2);
if(!(q<+br(D,B).toFixed(2)||q>+bs(D,B).toFixed(2)||q<+br(z,x).toFixed(2)||q>+bs(z,x).toFixed(2)||p<+br(C,A).toFixed(2)||p>+bs(C,A).toFixed(2)||p<+br(y,w).toFixed(2)||p>+bs(y,w).toFixed(2))){return{x:s,y:r}
...
}function aM(){return this.x+bC+this.y+bC+this.width+" Ã— "+this.height
}function aK(D,C,B,A,z,y){function x(b){return((s*b+t)*b+u)*b
}function w(e,d){var f=v(e,d);
return((p*f+q)*f+r)*f
}function v(j,g){var E,o,n,m,l,k;
for(n=j,k=0;
8>k;
k++){if(m=x(n)-j,bp(m)<g){return n
}if(l=(3*s*n+2*t)*n+u,bp(l)<0.000001){break
}n-=m/l
}if(E=0,o=1,n=j,E>n){return E
}if(n>o){return o
}for(;
o>E;
){if(m=x(n),bp(m-j)<g){return n
}j>m?E=n:o=n,n=(o-E)/2+E
}return n
}var u=3*C,t=3*(A-C)-u,s=1-u-t,r=3*B,q=3*(z-B)-r,p=1-r-q;
return w(D,1/(200*y))
...
j.title="RaphaÃ«l Colour Picker",j.style.display="none",bO.doc.body.appendChild(j),a9=a1(function(b){return j.style.color=b,bO.doc.defaultView.getComputedStyle(j,bE).getPropertyValue("color")
})
}return a9(f)
...
return k?(k[2]&&(o=bT(k[2].substring(5),16),p=bT(k[2].substring(3,5),16),c=bT(k[2].substring(1,3),16)),k[3]&&(o=bT((m=k[3].charAt(3))+m,16),p=bT((m=k[3].charAt(2))+m,16),c=bT((m=k[3].charAt(1))+m,16)),k[4]&&(l=k[4][bA](ah),c=bd(l[0]),"%"==l[0].slice(-1)&&(c*=2.55),p=bd(l[1]),"%"==l[1].slice(-1)&&(p*=2.55),o=bd(l[2]),"%"==l[2].slice(-1)&&(o*=2.55),"rgba"==k[1].toLowerCase().slice(0,4)&&(n=bd(l[3])),l[3]&&"%"==l[3].slice(-1)&&(n/=100)),k[5]?(l=k[5][bA](ah),c=bd(l[0]),"%"==l[0].slice(-1)&&(c*=2.55),p=bd(l[1]),"%"==l[1].slice(-1)&&(p*=2.55),o=bd(l[2]),"%"==l[2].slice(-1)&&(o*=2.55),("deg"==l[0].slice(-3)||"Â°"==l[0].slice(-1))&&(c/=360),"hsba"==k[1].toLowerCase().slice(0,4)&&(n=bd(l[3])),l[3]&&"%"==l[3].slice(-1)&&(n/=100),a6.hsb2rgb(c,p,o,n)):k[6]?(l=k[6][bA](ah),c=bd(l[0]),"%"==l[0].slice(-1)&&(c*=2.55),p=bd(l[1]),"%"==l[1].slice(-1)&&(p*=2.55),o=bd(l[2]),"%"==l[2].slice(-1)&&(o*=2.55),("deg"==l[0].slice(-3)||"Â°"==l[0].slice(-1))&&(c/=360),"hsla"==k[1].toLowerCase().slice(0,4)&&(n=bd(l[3])),l[3]&&"%"==l[3].slice(-1)&&(n/=100),a6.hsl2rgb(c,p,o,n)):(k={r:c,g:p,b:o,toString:aZ},k.hex="#"+(16777216|o|p<<8|c<<16).toString(16).slice(1),a6.is(n,"finite")&&(k.opacity=n),k)):{r:-1,g:-1,b:-1,hex:"none",error:1,toString:aZ}
...
},a6._removedFactory=function(b){return function(){a7("raphael.log",null,"RaphaÃ«l: you are calling to method â€œ"+b+"â€�? of removed object",b)
}
};
var bW=a6.pathBBox=function(H){var G=b8(H);
if(G.bbox){return a5(G.bbox)
}if(!H){return{x:0,y:0,width:0,height:0,x2:0,y2:0}
}H=ax(H);
for(var F,E=0,D=0,C=[],B=[],A=0,z=H.length;
z>A;
A++){if(F=H[A],"M"==F[0]){E=F[1],D=F[2],C.push(E),B.push(D)
}else{var y=a4(E,D,F[1],F[2],F[3],F[4],F[5],F[6]);
C=C[bI](y.min.x,y.max.x),B=B[bI](y.min.y,y.max.y),E=F[5],D=F[6]
}}var x=br[bK](0,C),w=br[bK](0,B),v=bs[bK](0,C),u=bs[bK](0,B),t=v-x,s=u-w,d={x:x,y:w,x2:v,y2:u,width:t,height:s,cx:x+t/2,cy:w+s/2};
return G.bbox=a5(d),d
},bz=function(d){var c=a5(d);
return c.toString=a6._path2string,c
},aW=a6._pathToRelative=function(H){var G=b8(H);
if(G.rel){return bz(G.rel)
}a6.is(H,bj)&&a6.is(H&&H[0],bj)||(H=a6.parsePathString(H));
var F=[],E=0,D=0,C=0,B=0,A=0;
"M"==H[0][0]&&(E=H[0][1],D=H[0][2],C=E,B=D,A++,F.push(["M",E,D]));
for(var z=A,y=H.length;
y>z;
z++){var x=F[z]=[],w=H[z];
if(w[0]!=bu.call(w[0])){switch(x[0]=bu.call(w[0]),x[0]){case"a":x[1]=w[1],x[2]=w[2],x[3]=w[3],x[4]=w[4],x[5]=w[5],x[6]=+(w[6]-E).toFixed(3),x[7]=+(w[7]-D).toFixed(3);
break;
case"v":x[1]=+(w[1]-D).toFixed(3);
break;
case"m":C=w[1],B=w[2];
default:for(var v=1,u=w.length;
u>v;
v++){x[v]=+(w[v]-(v%2?E:D)).toFixed(3)
}}}else{x=F[z]=[],"m"==w[0]&&(C=w[1]+E,B=w[2]+D);
for(var t=0,s=w.length;
s>t;
t++){F[z][t]=w[t]
}}var c=F[z].length;
switch(F[z][0]){case"z":E=C,D=B;
break;
case"h":E+=+F[z][c-1];
break;
case"v":D+=+F[z][c-1];
break;
default:E+=+F[z][c-2],D+=+F[z][c-1]
}}return F.toString=a6._path2string,G.rel=bz(F),F
...
},bH=function(t,s,r,q,p,o,n,m,l){var k=1-l;
return{x:bn(k,3)*t+3*bn(k,2)*l*r+3*k*l*l*p+bn(l,3)*n,y:bn(k,3)*s+3*bn(k,2)*l*q+3*k*l*l*o+bn(l,3)*m}
},a4=a1(function(F,E,D,C,B,A,z,y){var x,w=B-2*D+F-(z-2*B+D),v=2*(D-F)-2*(B-D),u=F-D,t=(-v+bt.sqrt(v*v-4*w*u))/2/w,s=(-v-bt.sqrt(v*v-4*w*u))/2/w,r=[E,y],q=[F,z];
return bp(t)>"1e12"&&(t=0.5),bp(s)>"1e12"&&(s=0.5),t>0&&1>t&&(x=bH(F,E,D,C,B,A,z,y,t),q.push(x.x),r.push(x.y)),s>0&&1>s&&(x=bH(F,E,D,C,B,A,z,y,s),q.push(x.x),r.push(x.y)),w=A-2*C+E-(y-2*A+C),v=2*(C-E)-2*(A-C),u=E-C,t=(-v+bt.sqrt(v*v-4*w*u))/2/w,s=(-v-bt.sqrt(v*v-4*w*u))/2/w,bp(t)>"1e12"&&(t=0.5),bp(s)>"1e12"&&(s=0.5),t>0&&1>t&&(x=bH(F,E,D,C,B,A,z,y,t),q.push(x.x),r.push(x.y)),s>0&&1>s&&(x=bH(F,E,D,C,B,A,z,y,s),q.push(x.x),r.push(x.y)),{min:{x:br[bK](0,q),y:br[bK](0,r)},max:{x:bs[bK](0,q),y:bs[bK](0,r)}}
}),ax=a6._path2curve=a1(function(F,E){var D=!E&&b8(F);
if(!E&&D.curve){return bz(D.curve)
}for(var C=av(F),B=E&&av(E),A={x:0,y:0,bx:0,by:0,X:0,Y:0,qx:null,qy:null},z={x:0,y:0,bx:0,by:0,X:0,Y:0,qx:null,qy:null},y=(function(g,f,j){var i,h;
...
}),x=function(e,d){if(e[d].length>7){e[d].shift();
for(var f=e[d];
f.length;
){e.splice(d++,0,["C"][bI](f.splice(0,6)))
}e.splice(d,1),u=bs(C.length,B&&B.length||0)
}},w=function(e,d,j,i,h){e&&d&&"M"==e[h][0]&&"M"!=d[h][0]&&(d.splice(h,0,["M",i.x,i.y]),j.bx=0,j.by=0,j.x=e[h][1],j.y=e[h][2],u=bs(C.length,B&&B.length||0))
},v=0,u=bs(C.length,B&&B.length||0);
u>v;
v++){C[v]=y(C[v],A),x(C,v),B&&(B[v]=y(B[v],z)),B&&x(B,v),w(C,B,A,z,v),w(B,C,z,A,v);
var t=C[v],s=B&&B[v],r=t.length,q=B&&s.length;
A.x=t[r-2],A.y=t[r-1],A.bx=bd(t[r-4])||A.x,A.by=bd(t[r-3])||A.y,z.bx=B&&(bd(s[q-4])||z.x),z.by=B&&(bd(s[q-3])||z.y),z.x=B&&s[q-2],z.y=B&&s[q-1]
}return B||(D.curve=bz(C)),B?[C,B]:C
},null,bz),am=(a6._parseDots=a1(function(t){for(var s=[],r=0,q=t.length;
q>r;
r++){var p={},o=t[r].match(/^([^:]*):?([\d\.]*)/);
if(p.color=a6.getRGB(o[1]),p.color.error){return null
}p.color=p.color.hex,o[2]&&(p.offset=o[2]+"%"),s.push(p)
}for(r=1,q=s.length-1;
q>r;
r++){if(!s[r].offset){for(var n=bd(s[r-1].offset||0),m=0,l=r+1;
q>l;
...
}},ba=a6._equaliseTransform=function(t,s){s=bB(s).replace(/\.{3}|\u2026/g,t),t=a6.parseTransformString(t)||[],s=a6.parseTransformString(s)||[];
for(var r,q,p,o,n=bs(t.length,s.length),m=[],l=[],c=0;
n>c;
c++){if(p=t[c]||bP(s[c]),o=s[c]||bP(p),p[0]!=o[0]||"r"==p[0].toLowerCase()&&(p[2]!=o[2]||p[3]!=o[3])||"s"==p[0].toLowerCase()&&(p[3]!=o[3]||p[4]!=o[4])){return
...
b[0]&&(b[0]/=d),b[1]&&(b[1]/=d)
}e.add=function(z,y,x,w,v,u){var t,s,r,q,p=[[],[],[]],o=[[this.a,this.c,this.e],[this.b,this.d,this.f],[0,0,1]],n=[[z,x,v],[y,w,u],[0,0,1]];
for(z&&z instanceof aN&&(n=[[z.a,z.c,z.e],[z.b,z.d,z.f],[0,0,1]]),t=0;
3>t;
t++){for(s=0;
3>s;
s++){for(q=0,r=0;
...
}(),ap=[],ae=function(B){for(var A,z=B.clientX,y=B.clientY,x=bO.doc.documentElement.scrollTop||bO.doc.body.scrollTop,w=bO.doc.documentElement.scrollLeft||bO.doc.body.scrollLeft,v=ap.length;
v--;
){if(A=ap[v],bF&&B.touches){for(var u,t=B.touches.length;
t--;
){if(u=B.touches[t],u.identifier==A.el._drag.id){z=u.clientX,y=u.clientY,(B.originalEvent?B.originalEvent:B).preventDefault();
break
}}}else{B.preventDefault()
}var s,r=A.el.node,q=r.nextSibling,p=r.parentNode,b=r.style.display;
bO.win.opera&&p.removeChild(r),r.style.display="none",s=A.el.paper.getElementByPoint(z,y),r.style.display=b,bO.win.opera&&(q?p.insertBefore(r,q):p.appendChild(r)),s&&a7("raphael.drag.over."+A.el.id,A.el,s),z+=w,y+=x,a7("raphael.drag.move."+A.el.id,A.move_scope||A.el,z-A.el._drag.x,y-A.el._drag.y,z,y,B)
...
},aq=function(d,c){return function(A,z,y){A=ax(A);
for(var x,w,v,u,t,s="",r={},q=0,b=0,a=A.length;
a>b;
b++){if(v=A[b],"M"==v[0]){x=+v[1],w=+v[2]
}else{if(u=aL(x,w,v[1],v[2],v[3],v[4],v[5],v[6]),q+u>z){if(c&&!r.start){if(t=aL(x,w,v[1],v[2],v[3],v[4],v[5],v[6],z-q),s+=["C"+t.start.x,t.start.y,t.m.x,t.m.y,t.x,t.y],y){return s
}r.start=s,s=["M"+t.x,t.y+"C"+t.n.x,t.n.y,t.end.x,t.end.y,v[5],v[6]].join(),q+=u,x=+v[5],w=+v[6];
continue
}if(!d&&!c){return t=aL(x,w,v[1],v[2],v[3],v[4],v[5],v[6],z-q),{x:t.x,y:t.y,alpha:t.alpha}
}}q+=u,x=+v[5],w=+v[6]
}s+=v.shift()+v
}return r.end=s,t=d?q:c?r:a6.findDotsAtSegment(x,w,v[0],v[1],v[2],v[3],v[4],v[5],1),t.alpha&&(t={x:t.x,y:t.y,alpha:t.alpha}),t
}
...
},ai=function(){for(var R=+new Date,Q=0;
Q<aS.length;
Q++){var P=aS[Q];
if(!P.el.removed&&!P.paused){var O,N,M=R-P.start,L=P.ms,K=P.easing,J=P.from,I=P.diff,H=P.to,G=(P.t,P.el),F={},E={};
if(P.initstatus?(M=(P.initstatus*P.anim.top-P.prev)/(P.percent-P.prev)*L,P.status=P.initstatus,delete P.initstatus,P.stop&&aS.splice(Q--,1)):P.status=(P.prev+(P.percent-P.prev)*(M/L))/P.anim.top,!(0>M)){if(L>M){var D=K(M/L);
for(var C in J){if(J[aA](C)){switch(ar[C]){case bl:O=+J[C]+D*L*I[C];
break;
case"colour":O="rgb("+[b9(b0(J[C].r+D*L*I[C].r)),b9(b0(J[C].g+D*L*I[C].g)),b9(b0(J[C].b+D*L*I[C].b))].join(",")+")";
break;
case"path":O=[];
for(var B=0,A=J[C].length;
A>B;
B++){O[B]=[J[C][B][0]];
for(var z=1,s=J[C][B].length;
s>z;
z++){O[B][z]=+J[C][B][z]+D*L*I[C][B][z]
}O[B]=O[B].join(bC)
}O=O.join(bC);
break;
case"transform":if(I[C].real){for(O=[],B=0,A=J[C].length;
A>B;
B++){for(O[B]=[J[C][B][0]],z=1,s=J[C][B].length;
s>z;
z++){O[B][z]=J[C][B][z]+D*L*I[C][B][z]
}}}else{var c=function(d){return +J[C][d]+D*L*I[C][d]
};
O=[["m",c(0),c(1),c(2),c(3),c(4),c(5)]]
}break;
case"csv":if("clip-rect"==C){for(O=[],B=4;
B--;
){O[B]=+J[C][B]+D*L*I[C][B]
}}break;
default:var b=[][bI](J[C]);
for(O=[],B=G.paper.customAttributes[C].length;
B--;
){O[B]=+b[B]+D*L*I[C][B]
}}F[C]=O
}}G.attr(F),function(e,g,f){setTimeout(function(){a7("raphael.anim.frame."+e,g,f)
})
}(G.id,G,P.anim)
...
}(P.callback,G,P.anim),G.attr(H),aS.splice(Q--,1),P.repeat>1&&!P.next){for(N in H){H[aA](N)&&(E[N]=P.totalOrigin[N])
}P.el.attr(E),aI(P.anim,P.el,P.anim.percents[0],null,P.totalOrigin,P.repeat-1)
}P.next&&!P.stop&&aI(P.anim,P.el,P.next,null,P.totalOrigin,P.repeat)
}}}}a6.svg&&G&&G.paper&&G.paper.safari(),aS.length&&at(ai)
},b9=function(b){return b>255?255:0>b?0:b
};
bJ.animateWith=function(t,s,r,q,p,o){var n=this;
...
c>l;
l++){if(aS[l].anim==s&&aS[l].el==t){aS[c-1].start=aS[l].start;
break
}}return n
...
},a7.on("raphael.remove",aH),a7.on("raphael.clear",aH),bJ.toString=function(){return"RaphaÃ«lâ€™s object"
};
...
}}},bD.animate=function(t,s,r,q){(a6.is(r,"function")||!r)&&(q=r||null);
var p,o,n=this.items.length,m=n,l=this;
if(!n){return this
}q&&(o=function(){!--n&&q.call(l)
}),r=a6.is(r,bk)?r:o;
var c=a6.animation(t,s,r,o);
...
},bD.toString=function(){return"RaphaÃ«lâ€˜s set"
...
},aF.getFont=function(t,s,r,q){if(q=q||"normal",r=r||"normal",s=+s||{normal:400,bold:700,lighter:300,bolder:800}[s]||400,a6.fonts){var p=a6.fonts[t];
if(!p){var o=new RegExp("(^|\\s)"+t.replace(/[^\w\d\s+!~.:_-]/g,bE)+"(\\s|$)","i");
for(var n in a6.fonts){if(a6.fonts[aA](n)&&o.test(n)){p=a6.fonts[n];
break
}}}var m;
if(p){for(var l=0,c=p.length;
c>l&&(m=p[l],m.face["font-weight"]!=s||m.face["font-style"]!=r&&m.face["font-style"]||m.face["font-stretch"]!=q);
l++){}}return m
...
}},F=function(ck,cj){var ci=ck.node,ch=ck.attrs,cg=ci.style.visibility;
ci.style.visibility="hidden";
for(var cc in cj){if(cj[bc](cc)){if(!a6._availableAttrs[bc](cc)){continue
}var cb=cj[cc];
switch(ch[cc]=cb,cc){case"blur":ck.blur(cb);
break;
case"href":case"title":var q=N("title"),j=a6._g.doc.createTextNode(cb);
q.appendChild(j),ci.appendChild(q);
break;
case"target":var g=ci.parentNode;
if("a"!=g.tagName.toLowerCase()){var q=N("a");
g.insertBefore(q,ci),q.appendChild(ci),g=q
}"target"==cc?g.setAttributeNS(R,"show","blank"==cb?"new":cb):g.setAttributeNS(R,cc,cb);
break;
case"cursor":ci.style.cursor=cb;
break;
case"transform":ck.transform(cb);
break;
case"arrow-start":I(ck,cb);
break;
case"arrow-end":I(ck,cb,1);
break;
case"clip-rect":var a=bb(cb).split(V);
if(4==a.length){ck.clip&&ck.clip.parentNode.parentNode.removeChild(ck.clip.parentNode);
var y=N("clipPath"),v=N("rect");
y.id=a6.createUUID(),N(v,{x:a[0],y:a[1],width:a[2],height:a[3]}),y.appendChild(v),ck.paper.defs.appendChild(y),N(ci,{"clip-path":"url(#"+y.id+")"}),ck.clip=v
}if(!cb){var t=ci.getAttribute("clip-path");
if(t){var s=a6._g.doc.getElementById(t.replace(/(^url\(#|\)$)/g,T));
s&&s.parentNode.removeChild(s),N(ci,{"clip-path":T}),delete ck.clip
}}break;
case"path":"path"==ck.type&&(N(ci,{d:cb?ch.path=a6._pathToAbsolute(cb):"M0,0"}),ck._.dirty=1,ck._.arrows&&("startString" in ck._.arrows&&I(ck,ck._.arrows.startString),"endString" in ck._.arrows&&I(ck,ck._.arrows.endString,1)));
break;
case"width":if(ci.setAttribute(cc,cb),ck._.dirty=1,!ch.fx){break
}cc="x",cb=ch.x;
case"x":ch.fx&&(cb=-ch.x-(ch.width||0));
case"rx":if("rx"==cc&&"rect"==ck.type){break
}case"cx":ci.setAttribute(cc,cb),ck.pattern&&J(ck),ck._.dirty=1;
break;
case"height":if(ci.setAttribute(cc,cb),ck._.dirty=1,!ch.fy){break
}cc="y",cb=ch.y;
case"y":ch.fy&&(cb=-ch.y-(ch.height||0));
case"ry":if("ry"==cc&&"rect"==ck.type){break
}case"cy":ci.setAttribute(cc,cb),ck.pattern&&J(ck),ck._.dirty=1;
break;
case"r":"rect"==ck.type?N(ci,{rx:cb,ry:cb}):ci.setAttribute(cc,cb),ck._.dirty=1;
break;
case"src":"image"==ck.type&&ci.setAttributeNS(R,"href",cb);
break;
case"stroke-width":(1!=ck._.sx||1!=ck._.sy)&&(cb/=Y(X(ck._.sx),X(ck._.sy))||1),ck.paper._vbSize&&(cb*=ck.paper._vbSize),ci.setAttribute(cc,cb),ch["stroke-dasharray"]&&G(ck,ch["stroke-dasharray"],cj),ck._.arrows&&("startString" in ck._.arrows&&I(ck,ck._.arrows.startString),"endString" in ck._.arrows&&I(ck,ck._.arrows.endString,1));
break;
case"stroke-dasharray":G(ck,cb,cj);
break;
case"fill":var r=bb(cb).match(a6._ISURL);
if(r){y=N("pattern");
var n=N("image");
y.id=a6.createUUID(),N(y,{x:0,y:0,patternUnits:"userSpaceOnUse",height:1,width:1}),N(n,{x:0,y:0,"xlink:href":r[1]}),y.appendChild(n),function(d){a6._preload(r[1],function(){var f=this.offsetWidth,i=this.offsetHeight;
N(d,{width:f,height:i}),N(n,{width:f,height:i}),ck.paper.safari()
})
}(y),ck.paper.defs.appendChild(y),N(ci,{fill:"url(#"+y.id+")"}),ck.pattern=y,ck.pattern&&J(ck);
break
}var l=a6.getRGB(cb);
if(l.error){if(("circle"==ck.type||"ellipse"==ck.type||"r"!=bb(cb).charAt())&&L(ck,cb)){if("opacity" in ch||"fill-opacity" in ch){var h=a6._g.doc.getElementById(ci.getAttribute("fill").replace(/^url\(#|\)$/g,T));
if(h){var e=h.getElementsByTagName("stop");
N(e[e.length-1],{"stop-opacity":("opacity" in ch?ch.opacity:1)*("fill-opacity" in ch?ch["fill-opacity"]:1)})
}}ch.gradient=cb,ch.fill="none";
break
}}else{delete cj.gradient,delete ch.gradient,!a6.is(ch.opacity,"undefined")&&a6.is(cj.opacity,"undefined")&&N(ci,{opacity:ch.opacity}),!a6.is(ch["fill-opacity"],"undefined")&&a6.is(cj["fill-opacity"],"undefined")&&N(ci,{"fill-opacity":ch["fill-opacity"]})
}l[bc]("opacity")&&N(ci,{"fill-opacity":l.opacity>1?l.opacity/100:l.opacity});
case"stroke":l=a6.getRGB(cb),ci.setAttribute(cc,l.hex),"stroke"==cc&&l[bc]("opacity")&&N(ci,{"stroke-opacity":l.opacity>1?l.opacity/100:l.opacity}),"stroke"==cc&&ck._.arrows&&("startString" in ck._.arrows&&I(ck,ck._.arrows.startString),"endString" in ck._.arrows&&I(ck,ck._.arrows.endString,1));
break;
case"gradient":("circle"==ck.type||"ellipse"==ck.type||"r"!=bb(cb).charAt())&&L(ck,cb);
break;
case"opacity":ch.gradient&&!ch[bc]("stroke-opacity")&&N(ci,{"stroke-opacity":cb>1?cb/100:cb});
case"fill-opacity":if(ch.gradient){h=a6._g.doc.getElementById(ci.getAttribute("fill").replace(/^url\(#|\)$/g,T)),h&&(e=h.getElementsByTagName("stop"),N(e[e.length-1],{"stop-opacity":cb}));
break
}default:"font-size"==cc&&(cb=ab(cb,10)+"px");
var b=cc.replace(/(\-.)/g,function(d){return d.substring(1).toUpperCase()
});
ci.style[b]=cb,ck._.dirty=1,ci.setAttribute(cc,cb)
}}}D(ck,cj),ci.style.visibility=cg
},E=1.2,D=function(y,x){if("text"==y.type&&(x[bc]("text")||x[bc]("font")||x[bc]("font-size")||x[bc]("x")||x[bc]("y"))){var w=y.attrs,v=y.node,u=v.firstChild?ab(a6._g.doc.defaultView.getComputedStyle(v.firstChild,T).getPropertyValue("font-size"),10):10;
if(x[bc]("text")){for(w.text=x.text;
v.firstChild;
){v.removeChild(v.firstChild)
}for(var t,s=bb(x.text).split("\n"),q=[],l=0,e=s.length;
e>l;
l++){t=N("tspan"),l&&N(t,{dy:u*E,x:w.x}),t.appendChild(a6._g.doc.createTextNode(s[l])),v.appendChild(t),q[l]=t
}}else{for(q=v.getElementsByTagName("tspan"),l=0,e=q.length;
e>l;
l++){l?N(q[l],{dy:u*E,x:w.x}):N(q[0],{dy:0})
}}N(v,{x:w.x,y:w.y}),y._.dirty=1;
var b=y._getBBox(),a=w.y-(b.y+b.height/2);
...
},O.attr=function(y,x){if(this.removed){return this
}if(null==y){var w={};
for(var v in this.attrs){this.attrs[bc](v)&&(w[v]=this.attrs[v])
}return w.gradient&&"none"==w.fill&&(w.fill=w.gradient)&&delete w.gradient,w.transform=this._.transform,w
}if(null==x&&a6.is(y,"string")){if("fill"==y&&"none"==this.attrs.fill&&this.attrs.gradient){return this.attrs.gradient
}if("transform"==y){return this._.transform
}for(var u=y.split(V),t={},s=0,r=u.length;
r>s;
s++){y=u[s],t[y]=y in this.attrs?this.attrs[y]:a6.is(this.paper.customAttributes[y],"function")?this.paper.customAttributes[y].def:a6._availableAttrs[y]
}return r-1?t:t[u[0]]
}if(null==x&&a6.is(y,"array")){for(t={},s=0,r=y.length;
r>s;
s++){t[y[s]]=this.attr(y[s])
}return t
}if(null!=x){var q={};
q[y]=x
}else{null!=y&&a6.is(y,"object")&&(q=y)
...
},a6._engine.create=function(){var s=a6._getContainer.apply(0,arguments),r=s&&s.container,q=s.x,p=s.y,o=s.width,n=s.height;
...
},a6._engine.setViewBox=function(t,s,r,q,p){U("raphael.setViewBox",this,this._viewBox,[t,s,r,q,p]);
var o,n,m=Y(r/this.width,q/this.height),k=this.top,g=p?"meet":"xMinYMin";
for(null==t?(this._vbSize&&(m=1),delete this._vbSize,o="0 0 "+this.width+S+this.height):(this._vbSize=m,o=t+S+s+S+r+S+q),N(this.canvas,{viewBox:o,preserveAspectRatio:g});
m&&k;
){n="stroke-width" in k.attrs?k.attrs["stroke-width"]:1,k.attr({"stroke-width":n}),k._.dirty=1,k._.dirtyT=1,k=k.prev
}return this._viewBox=[t,s,r,q,!!p],this
...
}this.bottom=this.top=null,(this.desc=N("desc")).appendChild(a6._g.doc.createTextNode("Created with RaphaÃ«l "+a6.version)),b.appendChild(this.desc),b.appendChild(this.defs=N("defs"))
...
}}}(),function(){if(a6.vml){var cn="hasOwnProperty",cm=String,cl=parseFloat,ck=Math,cj=ck.round,ci=ck.max,ch=ck.min,cg=ck.abs,cc="fill",cb=/[, ]+/,bc=a6.eve,bb=" progid:DXImageTransform.Microsoft",ac=" ",ab="",Z={M:"m",L:"l",C:"c",Z:"x",m:"t",l:"r",c:"v",z:"x"},X=/([clmz]),?([^clmz]*)/gi,V=/ progid:\S+Blur\([^\)]+\)/g,T=/-?[^,\s-]+/g,R="position:absolute;left:0;top:0;width:1px;height:1px",P=21600,N={path:1,rect:1,image:1},L={circle:1,ellipse:1},J=function(w){var v=/[ahqstv]/gi,u=a6._pathToAbsolute;
if(cm(w).match(v)&&(u=a6._path2curve),v=/[clmz]/g,u==a6._pathToAbsolute&&!cm(w).match(v)){var t=cm(w).replace(X,function(i,h,m){var l=[],k="m"==h.toLowerCase(),j=Z[h];
...
n>o;
o++){s=p[o],q=p[o][0].toLowerCase(),"z"==q&&(q="x");
for(var f=1,b=s.length;
b>f;
f++){q+=cj(s[f]*P)+(f!=b-1?",":ab)
}t.push(q)
}return t.join(ac)
...
a6.toString=function(){return"Your browser doesnâ€™t support SVG. Falling down to VML.\nYou are running RaphaÃ«l "+this.version
};
var Y=function(t,s,r){for(var q=cm(s).toLowerCase().split("-"),p=r?"end":"start",o=q.length,n="classic",m="medium",l="medium";
o--;
...
}}var b=t.node.getElementsByTagName("stroke")[0];
...
}},U=function(w,v,u){w.attrs=w.attrs||{};
var t=(w.attrs,Math.pow),s="linear",r=".5 .5";
if(w.attrs.gradient=v,v=cm(v).replace(a6._radial_gradient,function(g,f,h){return s="radial",f&&h&&(f=cl(f),h=cl(h),t(f-0.5,2)+t(h-0.5,2)>0.25&&(h=ck.sqrt(0.25-t(f-0.5,2))*(2*(h>0.5)-1)+0.5),r=f+ac+h),ab
}),v=v.split(/\s*\-\s*/),"linear"==s){var o=v.shift();
if(o=-cl(o),isNaN(o)){return null
}}var n=a6._parseDots(v);
if(!n){return null
}if(w=w.shape||w.node,n.length){w.removeChild(u),u.on=!0,u.method="none",u.color=n[0].color,u.color2=n[n.length-1].color;
for(var e=[],d=0,b=n.length;
b>d;
d++){n[d].offset&&e.push(n[d].offset+ac+n[d].color)
}u.colors=e.length?e.join():"0% "+u.color,"radial"==s?(u.type="gradientTitle",u.focus="100%",u.focussize="0 0",u.focusposition=r,u.angle=0):(u.type="gradient",u.angle=(270-o)%360),w.appendChild(u)
}return 1
...
S.prototype=Q,Q.constructor=S,Q.transform=function(C){if(null==C){return this._.transform
}var B,A=this.paper._viewBoxShift,z=A?"s"+[A.scale,A.scale]+"-1-1t"+[A.dx,A.dy]:ab;
A&&(B=C=cm(C).replace(/\.{3}|\u2026/g,this._.transform||ab)),a6._extractTransform(this,z+C);
var y,x=this.matrix.clone(),w=this.skew,v=this.node,u=~cm(this.attrs.fill).indexOf("-"),t=!cm(this.attrs.fill).indexOf("url(");
if(x.translate(1,1),t||u||"image"==this.type){if(w.matrix="1 0 0 1",w.offset="0 0",y=x.split(),u&&y.noRotation||!y.isSimple){v.style.filter=x.toFilter();
var s=this.getBBox(),o=this.getBBox(1),n=s.x-o.x,b=s.y-o.y;
v.coordorigin=n*-P+ac+b*-P,c(this,1,1,n,b,0)
}else{v.style.filter=ab,c(this,y.scalex,y.scaley,y.dx,y.dy,y.rotate)
}}else{v.style.filter=ab,w.matrix=cm(x),w.offset=x.offset()
...
}},Q.attr=function(y,x){if(this.removed){return this
}if(null==y){var w={};
for(var v in this.attrs){this.attrs[cn](v)&&(w[v]=this.attrs[v])
}return w.gradient&&"none"==w.fill&&(w.fill=w.gradient)&&delete w.gradient,w.transform=this._.transform,w
}if(null==x&&a6.is(y,"string")){if(y==cc&&"none"==this.attrs.fill&&this.attrs.gradient){return this.attrs.gradient
}for(var u=y.split(cb),t={},s=0,r=u.length;
r>s;
s++){y=u[s],t[y]=y in this.attrs?this.attrs[y]:a6.is(this.paper.customAttributes[y],"function")?this.paper.customAttributes[y].def:a6._availableAttrs[y]
}return r-1?t:t[u[0]]
}if(this.attrs&&null==x&&a6.is(y,"array")){for(t={},s=0,r=y.length;
r>s;
s++){t[y[s]]=this.attr(y[s])
}return t
}var l;
null!=x&&(l={},l[y]=x),null==x&&a6.is(y,"object")&&(l=y);
...
},a6._engine.rect=function(s,r,q,p,o,n){var m=a6._rectPath(r,q,p,o,n),l=s.path(m),k=l.attrs;
...
},a6._engine.image=function(w,v,u,t,s,r){var q=a6._rectPath(u,t,s,r),p=w.path(q).attr({stroke:"none"}),o=p.attrs,n=p.node,j=n.getElementsByTagName(cc)[0];
return o.src=v,p.X=o.x=u,p.Y=o.y=t,p.W=o.width=s,p.H=o.height=r,o.path=q,p.type="image",j.parentNode==n&&n.removeChild(j),j.rotate=!0,j.src=v,j.type="tile",p._.fillpos=[u,t],p._.fillsize=[s,r],n.appendChild(j),c(p,1,1,0,0,0),p
},a6._engine.text=function(u,t,s,r){var q=O("shape"),p=O("path"),o=O("textpath");
t=t||0,s=s||0,r=r||"",p.v=a6.format("m{0},{1}l{2},{1}",cj(t*P),cj(s*P),cj(t*P)+1),p.textpathok=!0,o.string=cm(r),o.on=!0,q.style.cssText=R,q.coordsize=P+ac+P,q.coordorigin="0 0";
var n=new S(q,u),f={fill:"#000",stroke:"none",font:a6._availableAttrs.font,text:r};
n.shape=q,n.path=p,n.textpath=o,n.type="text",n.attrs.text=cm(r),n.attrs.x=t,n.attrs.y=s,n.attrs.w=1,n.attrs.h=1,W(n,f),q.appendChild(o),q.appendChild(p),u.canvas.appendChild(q);
var b=O("skew");
...
},a6._engine.setViewBox=function(u,t,s,r,q){a6.eve("raphael.setViewBox",this,this._viewBox,[u,t,s,r,q]);
var p,o,n=this.width,m=this.height,g=1/ci(s/n,r/m);
return q&&(p=m/r,o=n/s,n>s*p&&(u-=(n-s*p)/2/p),m>r*o&&(t-=(m-r*o)/2/o)),this._viewBox=[u,t,s,r,!!q],this._viewBoxShift={dx:-u,dy:-t,scale:g},this.forEach(function(b){b.transform("...")
}),this
};
var O;
...
}},a6._engine.initWin(a6._g.win),a6._engine.create=function(){var s=a6._getContainer.apply(0,arguments),r=s.container,q=s.height,p=s.width,o=s.x,n=s.y;
